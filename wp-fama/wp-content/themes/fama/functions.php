...
if ( function_exists( 'add_image_size' ) ) { 
  add_image_size( 'nota-huge', 970, 550 , true ); // home main note
  add_image_size( 'nota-large', 548, 394 , false ); // home main note
...
function pd_home_settings_menu() {
  add_posts_page('Configuración de Notas - Página Principal', 'Configuración', 'edit_posts', 'pd-home-settings', 'pd_home_settings_page_fn');
}
...
function post_agencia_meta() {
  add_meta_box( 'post_meta1', 'Fuente de Información', 'post_agencia_nombre_meta', 'post', 'side', 'high' );
  add_meta_box( 'post_meta2', 'Previo', 'post_balazo_meta', 'post', 'side', 'high' );
}


function post_agencia_nombre_meta( $post ) {
  $post_agencia_nombre = get_post_meta( $post->ID, '_post_agencia_nombre', true);
  echo 'Capture la fuente de información';
?>
...
function post_balazo_meta( $post ) {
  ...
  echo 'Capture el texto del previo';
?>
